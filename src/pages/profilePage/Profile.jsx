...
export default function Profile() {
  ...
  const addUser = function (e) {
    ...
    setFormData({
      ...
      signupPhoneNumber: localStorage.getItem('signupPhoneNumber')
    });
    console.log(formData)
    axios.post('https://ecommerece-backend.vercel.app/api/v1/users', { formData, finalAddress })
      .then(
        (res) => {
  ...
  // console.log(localStorage.getItem('signupPhoneNumber'))
  ...
  useEffect(
    () => {
      axios.post('https://ecommerece-backend.vercel.app/api/v1/existinguser', { signupPhoneNumber: localStorage.getItem('signupPhoneNumber') })
        .then(
          (res) => {
            setUser(res.data);
          }
            ...
            <div id='myDetails' className='flex flex-col w-full gap-5 px-8 py-6'>
              ...
              <div className='flex flex-col gap-4 justify-center'>
                ...
                {user.length <= 0 || user.length == undefined ? <div className='flex my-4 px-12 gap-4'>
                  <span onClick={() => localStorage.setItem('gender' , 'male')} className={`w-20 flex flex-col text-blue-400 hover:bg-blue-100 cursor-pointer items-center justify-center border-blue-400 rounded-xl h-20 border ${localStorage.getItem('gender') === 'male' ? 'bg-blue-100 text-blue-500' : ''}`}>
                    <MaleIcon />
                    <p>Male</p>
                  </span>
                  <span onClick={() => localStorage.setItem('gender' , 'female')} className={`w-20 flex flex-col text-blue-400 hover:bg-pink-100 cursor-pointer items-center justify-center border-blue-400 rounded-xl h-20 border ${localStorage.getItem('gender') === 'female' ? 'bg-pink-100 text-pink-500' : ''}`}>
                    <FemaleIcon />
                    <p>Female</p>
                  </span>
                </div> : ''
